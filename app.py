...
.status-dot.recording{background:#e53e3e;animation:pulse 1.5s infinite}
.status-dot.loading{background:#ed8936}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}
.mic-bars{display:flex;align-items:flex-end;gap:3px;height:20px;flex-shrink:0}
.mic-bars .bar{width:4px;border-radius:2px;background:#2d3048;transition:height .1s ease,background .1s ease}
.mic-bars.active .bar{background:#48bb78}
.badge{font-size:.75rem;color:#a0aec0}
...
.btn-blue{background:#2b4c8c;color:#fff}
.btn-sm{padding:.3rem .7rem;font-size:.8rem}
.tabs{display:flex;gap:2px;background:#1a1d27;border-bottom:1px solid #2d3048;
      padding:0 1rem;overflow-x:auto}
.tab{padding:.65rem 1rem;cursor:pointer;color:#718096;font-size:.85rem;white-space:nowrap;
     border-bottom:2px solid transparent;transition:color .15s;flex-shrink:0}
.tab.active{color:#e2e8f0;border-bottom-color:#667eea}
.panel{display:none;padding:1rem 1.2rem}
.panel.active{display:block}
.feed{max-height:calc(100vh - 210px);overflow-y:auto}
.entry{padding:.55rem .75rem;margin-bottom:.35rem;background:#1a1d27;
       border-radius:6px;border-left:3px solid #2d3048}
.entry.unprocessed{border-left-color:#667eea}
.ts{font-size:.7rem;color:#718096;margin-bottom:.15rem}
.txt{font-size:.88rem;line-height:1.5}
ul.checklist{list-style:none}
ul.checklist li{display:flex;align-items:center;gap:.55rem;padding:.5rem .75rem;
                margin-bottom:.3rem;background:#1a1d27;border-radius:6px}
ul.checklist li.done .label{text-decoration:line-through;color:#4a5568}
.label{flex:1;font-size:.88rem}
.qty{font-size:.78rem;color:#718096;margin-right:.4rem}
.cb{width:16px;height:16px;cursor:pointer;accent-color:#667eea;flex-shrink:0}
.del{background:none;border:none;color:#4a5568;cursor:pointer;font-size:.95rem;padding:0 .15rem}
.del:hover{color:#e53e3e}
.toolbar{display:flex;gap:.5rem;margin-bottom:.8rem;align-items:center;flex-wrap:wrap}
.empty{color:#4a5568;font-size:.88rem;padding:.8rem 0}
.cat-badge{font-size:.7rem;padding:.1rem .4rem;border-radius:10px;background:#2d3748;
           color:#a0aec0;flex-shrink:0}
/* summaries */
.summary-card{background:#1a1d27;border-radius:8px;margin-bottom:.7rem;overflow:hidden}
.summary-header{padding:.6rem .8rem;display:flex;align-items:flex-start;gap:.6rem}
.summary-title{flex:1;font-size:.9rem;font-weight:500;line-height:1.4}
.summary-meta{font-size:.72rem;color:#718096;padding:0 .8rem .4rem}
.summary-body{font-size:.85rem;line-height:1.6;color:#cbd5e0;
              padding:.5rem .8rem .8rem;border-top:1px solid #2d3048;white-space:pre-wrap}
/* extractions log */
.ext-entry{background:#1a1d27;border-radius:6px;margin-bottom:.5rem;overflow:hidden}
.ext-header{padding:.45rem .75rem;display:flex;gap:.8rem;align-items:center;
            font-size:.78rem;color:#718096;border-bottom:1px solid #2d3048}
.ext-counts{margin-left:auto;color:#a0aec0}
.ext-raw{padding:.55rem .75rem;font-family:monospace;font-size:.8rem;white-space:pre-wrap;
         word-break:break-word;color:#c3dafe;max-height:260px;overflow-y:auto}
/* settings */
.section-title{font-size:.8rem;font-weight:600;letter-spacing:.06em;text-transform:uppercase;
               color:#718096;margin-bottom:.6rem}
.agent-card{background:#1a1d27;border-radius:8px;padding:.85rem 1rem;margin-bottom:.6rem;
            border:1px solid #2d3048}
.agent-header{display:flex;align-items:center;gap:.6rem;margin-bottom:.4rem}
.agent-name{font-weight:600;font-size:.92rem;flex:1}
.source-badge{font-size:.7rem;padding:.1rem .45rem;border-radius:10px;flex-shrink:0}
.src-transcriptions{background:#2a3f6f;color:#90cdf4}
.src-hackernews{background:#4a2e08;color:#fbd38d}
.agent-desc{font-size:.82rem;color:#a0aec0;margin-bottom:.65rem}
.agent-footer{display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;margin-top:.6rem}
.last-run{font-size:.75rem;color:#718096;flex:1}
/* toggle switch */
.toggle{position:relative;display:inline-block;width:38px;height:22px;flex-shrink:0}
.toggle input{opacity:0;width:0;height:0}
.slider{position:absolute;cursor:pointer;inset:0;background:#4a5568;border-radius:22px;
        transition:.25s}
...
/* interval row */
.interval-row{display:flex;align-items:center;gap:.5rem;margin-top:.5rem;flex-wrap:wrap}
.interval-row label{font-size:.8rem;color:#a0aec0}
.interval-select{background:#2d3048;color:#e2e8f0;border:1px solid #4a5568;
                 border-radius:5px;padding:.3rem .5rem;font-size:.82rem;cursor:pointer}
.interval-custom{width:70px;background:#2d3048;color:#e2e8f0;border:1px solid #4a5568;
                 border-radius:5px;padding:.3rem .5rem;font-size:.82rem;display:none}
/* notif card */
.notif-card{background:#1a1d27;border-radius:8px;padding:.85rem 1rem;margin-bottom:1rem;
            border:1px solid #2d3048}
.notif-status{font-size:.85rem;color:#a0aec0;margin:.4rem 0 .7rem}
/* toast */
#toast{position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);
       background:#2d3748;color:#e2e8f0;padding:.65rem 1.3rem;border-radius:8px;
       font-size:.88rem;box-shadow:0 4px 16px rgba(0,0,0,.5);opacity:0;
       transition:opacity .3s;pointer-events:none;z-index:999;white-space:nowrap}
#toast.show{opacity:1}
#toast.ok{border-left:4px solid #48bb78}
#toast.warn{border-left:4px solid #ed8936}
#toast.err{border-left:4px solid #e53e3e}
</style>
</head>
<body>
<header>
